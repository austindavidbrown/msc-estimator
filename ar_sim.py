...
def sim_mcmc():
  run_lengths = np.zeros(N)

  init_xs = m + h**(1/2) * np.random.normal(size = (N, d))
  init_probs = self_normalized_importance_weights(init_xs)
  init_indices = np.random.choice(np.arange(0, N), 
                                  N, 
                                  p=init_probs, 
                                  replace = True)
  init_xs = init_xs[init_indices]
  ...
  for t in range(1, 10**6):
    # sample
    x_new = np.zeros((N, d))
    xi = np.random.normal(size = x[update_indices].shape, loc = 0, scale = 1)
...
true_samples = np.random.normal(size = (N, d), loc = m, scale = 1)
...
plt.xlabel("N sample", fontsize=30)
plt.ylabel("Run length", fontsize=30)
plt.savefig("pg_runlength.pdf", 
            format='pdf', 
            pad_inches=0, 
            bbox_inches='tight')
...
for k in range(0, d):
  ...
  plt.tick_params(axis="x", labelsize=20)
  plt.tick_params(axis="y", labelsize=20)
  plt.xlabel("N iterations", fontsize=30)
